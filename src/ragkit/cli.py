...
def _paths(base_dir: str) -> Tuple[str, str, str]:
    eat_default = os.path.join(base_dir, "prompts", "rag_assistant.eat")
    corpus_default = os.path.join(base_dir, "examples", "corpus")
    eval_default = os.path.join(base_dir, "eval", "rag_eval_set.csv")
    eat = os.environ.get("RAGKIT_EAT", eat_default)
    corpus = os.environ.get("RAGKIT_CORPUS", corpus_default)
    csv = os.environ.get("RAGKIT_EVAL", eval_default)
...
def main(argv: Optional[List[str]] = None, base_dir: Optional[str] = None) -> int:
    ...
    eat, _, _ = _paths(base_dir)
    if cmd in {"validate", "prompt", "ask", "eval", "all"}:
        if not os.path.exists(eat):
            print(
                f"Could not find the EAT profile at {eat}.\n"
                "Run this from the repository root, or set RAGKIT_EAT, "
                "RAGKIT_CORPUS and RAGKIT_EVAL to point at your files.",
                file=sys.stderr,
            )
            return 2
